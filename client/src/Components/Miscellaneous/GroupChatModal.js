...
const GroupChatModal = ({ children }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const { data } = await axios.post(`/api/chat/addGroup`, { name: groupChatName, participants: selectedUsers.map((u) => u._id), }, { headers: { "user-info": localStorage.getItem("user-info") } });
            console.log(data);
            setChats([data, ...chats]);
            onClose();
        ...
        <>
            ...
            <Modal onClose={onClose} isOpen={isOpen} isCentered>
                ...
                <ModalContent>
                    ...
                    <ModalCloseButton />
                    <ModalBody isplay="flex" flexDir="column" alignItems="center">
                        <FormControl>
                        ...
                        {loading ? (
                            ...
                            searchResult?.slice(0, 4).map((ele) => (<UserListItem key={ele._id} ele={ele} handleFunction={() => handleGroup(ele)} />))
                        )}
                    </ModalBody>
                    <ModalFooter>
